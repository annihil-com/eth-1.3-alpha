...
void __attribute__ ((constructor)) ethInit(void) {
	...
	if (!strcmp(exeName, "et.x86")) {
		...
		if (!getenv("ETH_SELFKILLSPAM"))
			strncpy(skFormat, "^3!!^7Holy Shit...SuIcide^3!!",sizeof(skFormat));
		else
			strncpy(skFormat, getenv("ETH_SELFKILLSPAM"),sizeof(skFormat));

		if (!getenv("ETH_KILLSPAM"))
			strncpy(ksFormat, "^1[v] ^7was owned by the ^nPenguin..^3(kills: ^1[t]^3) [m]",sizeof(ksFormat));
		else
			strncpy(ksFormat,getenv("ETH_KILLSPAM"),sizeof(ksFormat));

// Set banner/stats format
		if (!getenv("ETH_BANNER"))
			strncpy(bnFormat,"^2|^7Name: ^x[p]^* ^2| ^3[M] ^2| ^7Kills: ^1[t] ^2| ^7Spree: ^1[c] ^2| ^7Xp: ^1[x]",sizeof(bnFormat));
		else
			strncpy(bnFormat,getenv("ETH_BANNER"),sizeof(bnFormat));

// Set deathspam
		if (!getenv("ETH_DEATHSPAM"))
			strncpy(dieFormat,"^7Damn ^n[K] ^7Killed me.",sizeof(dieFormat));
		else
			strncpy(dieFormat,getenv("ETH_DEATHSPAM"),sizeof(dieFormat));

		fillHitboxCorrection();
	}
}
		...
		case KeyPress:
			switch (XLookupKeysym(&event->xkey, 0)) {
				case ETH_MENU_KEY:
					...
					break;
				case ETH_CAM_DROPKEY:
					if( eth.hookLoad && eth.cg_snap ){
						VectorCopy(eth.cg_refdefViewAngles, eth.dropCamRefDef[0]);
						VectorCopy(eth.cg_snap->ps.origin, eth.dropCamRefDef[1]);
						char msg[512];
						snprintf(msg,sizeof(msg),"^nNew origin ^0{^2%.1f, %.1f, %.1f^0} ^0{^2%.1f, %.1f, %.1f^0}\n",
								 eth.dropCamRefDef[1][0], eth.dropCamRefDef[1][1], eth.dropCamRefDef[1][2],
		 						 eth.dropCamRefDef[0][0], eth.dropCamRefDef[0][1], eth.dropCamRefDef[0][2] );
...
int eth_CG_syscall(int command, ...) {
	...
	switch (command) {
		case CG_GETSNAPSHOT: {
			...
			if( seth.value[VAR_AIMPREDICT] == AIMPREDICT_SNAPSHOT || seth.value[VAR_ENTCAM] == ENTCAM_CLOSEST
						 || seth.value[VAR_ENTCAM] == ENTCAM_SNIPER ){
				int i = 0;
				ethEntity_t *ent;
				int entNum;
				float mind = 2.0e6, s;
				int mind_ent = -1;
				int msec = orig_syscall( CG_MILLISECONDS );
				if( seth.value[VAR_ENTCAM] == ENTCAM_SNIPER )
					eth.entCamTarget = -1;	
				
				for(;i<eth.cg_snap->numEntities;i++){
					entNum = eth.cg_snap->entities[i].number;
					if( entNum < 0 || entNum > MAX_CLIENTS )
						continue;
					
					ent = &eth.entities[entNum];

					// fixme: use pointers and an index instead
					if( seth.value[VAR_AIMPREDICT] == AIMPREDICT_SNAPSHOT ){
						VectorCopy(history[entNum][1].trPos,history[entNum][2].trPos);
						VectorCopy(history[entNum][0].trPos,history[entNum][1].trPos);
						VectorCopy(eth.cg_snap->entities[i].pos.trBase, history[entNum][0].trPos);
						history[entNum][2].t = history[entNum][1].t;
						history[entNum][1].t = history[entNum][0].t;
						history[entNum][0].t = (float )msec;
					}

					if( seth.value[VAR_ENTCAM] == ENTCAM_CLOSEST && IS_PLAYER_ENEMY(entNum) ) {
						s = VectorDistanceSquared( eth.cg_snap->ps.origin, eth.cg_snap->entities[i].pos.trBase );
						if( s < mind ){
							mind = s;
							mind_ent = entNum;
						}
					...
					if( seth.value[VAR_ENTCAM] == ENTCAM_SNIPER  && IS_PLAYER_ENEMY(entNum) ){
						int weap = eth.cg_snap->entities[i].weapon;
						if( eth.mod->type == MOD_NQMOD )
							weap = NQWeapon2BaseWeapon( weap );
						else if( eth.mod->type == MOD_GAYMOD )
							weap = GaymodWeapon2BaseWeapon( weap );
						if( weap == WP_GARAND_SCOPE || weap == WP_K43_SCOPE )
							eth.entCamTarget = entNum;
					}
				}

				if( seth.value[VAR_ENTCAM] == ENTCAM_CLOSEST )
					eth.entCamTarget = mind_ent;
			}
			...
			if (!strcmp(cmd, "chat")) {
				...
				if ((fromPlayer != eth.cg_clientNum) && str) {
					...
					if (!strcasecmp(str, "Medic")
							&& (VectorDistance(eth.cg_refdef.vieworg, eth.entities[fromPlayer].origin) < 500.0f)) {
...
void initEthCgameStruct() {
	...
	if (!eth.mod->CG_WeaponFireRecoil)
		...
		// CG_Missile()
	if( eth.mod->CG_Missile ){
		eth.CG_Missile = detourFunction((void *)(eth.cgameLibAddress + eth.mod->CG_Missile), eth_CG_Missile);
		if( !eth.mod->cgs )
			eth.cgs = (cgs_t *) dlsym(eth.cgameLibHandle, "cgs");
	}
